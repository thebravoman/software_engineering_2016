...
class ArticlesController < ApplicationController
  ...
  def index
    ...
    @articles.each do |article|
      array << {name: article.name, id: article.id, created_at: article.created_at, update_at: article.updated_at}
    end
    output = {articles: array.sort_by { |hash| hash[:name]}}
  ...
  def get_comments
      ...
      output = {articles: comments_array.sort_by {|comment| comment[:content]}}
      render :json => output.to_json
  end
  def get_score
      vote_score = 0
      Vote.all.where("article_id = ?", params[:id]).each do |vote|
          vote_score = vote_score + vote.value
      end
      comment_score = 0
      Comment.all.where("article_id = ?", params[:id]).each do |comment|
      ...
      set_article
      output = {id: @article.id, name: @article.name,
  ...
  def sort_by_score
      ...
      output = articles.sort_by {|article| -article[:total_score]}
      render :json => output.to_json
  end
  private
    def set_article
