require "nzmsrt/version"


class SRTParser
   def number_of_words (line)
   		return line.scan(/[A-Za-z]+/).size
   end
   ...
   def average_symbols_per_line (parsed_hash)
   		parsed_hash["average_symbols_per_line"] = parsed_hash["number_of_symbols"].to_i/parsed_hash["number_of_lines"].to_f
   		return parsed_hash
   end
   ...
   def duration (line)
   		  	line = line.gsub!(/-->/," ").gsub!(/,/,".").gsub!(/:/," ").split(" ")
   		  	
   		  	is_right = line[3] > line[0] || line[4] > line[1]
   		  	
   		  	if is_right 
   		  		return ((line[5].to_f - line[2].to_f) + 60).round()
   		  	else 
   		  		return (line[5].to_f - line[2].to_f).round()
   		  	end
   end
   ...
   public def parse_file(path_to_file)
   		...
   		File.open(path_to_file, "r") do |file_line|
   			file_line.each_line() do |str_line|
   				...
   				if str_line =~ /-->/
   					parsed_hash["duration"] += duration(str_line)
   				end
